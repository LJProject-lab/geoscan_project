...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['token'], $_POST['pin'])) {
    // Get token and new password
    $token = $_POST['token'];
    $new_pin = password_hash($_POST['pin'], PASSWORD_BCRYPT); // Hash the new password

    // Check if token is valid and not expired
    $stmt = $pdo->prepare('SELECT * FROM tbl_users WHERE reset_token = :token AND token_expiry > NOW()');
    $stmt->execute(['token' => $token]);
    $user = $stmt->fetch(PDO::FETCH_ASSOC);

    if ($user) {
        // Update password
        $stmt = $pdo->prepare('UPDATE tbl_users SET pin = :pin, reset_token = NULL, token_expiry = NULL WHERE reset_token = :token');
        $stmt->execute([
            'pin' => $new_pin,
            'token' => $token
        ]);

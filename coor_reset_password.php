...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['token'], $_POST['password'])) {
    ...
    $stmt = $pdo->prepare('SELECT * FROM tbl_coordinators WHERE reset_token = :token AND token_expiry > NOW()');
    $stmt->execute(['token' => $token]);
    $user = $stmt->fetch(PDO::FETCH_ASSOC);

    if ($user) {
        // Update password
        $stmt = $pdo->prepare('UPDATE tbl_coordinators SET password = :password, reset_token = NULL, token_expiry = NULL WHERE reset_token = :token');
        $stmt->execute([
    ...
    <div id="internLoginCard" class="col-lg-3 card-container">
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="coor_reset_password.php" method="POST">
